...
* `r sum(tabs[,,1])` species in consensus tree (complete genetic data); `r sum(tabs[,,2])` in "tree block" (imputations)
...
* now apply your favourite optimization (or sampling) algorithm ...

## Likelihood ratio tests

* via `corHMM` package [@corHMM; @beaulieu_hidden_2014]
...
Continuous/quantitative (estimation w/ uncertainty)	MLE etc. + confidence intervals [@cummingIntroduction2017]	posterior means/medians + credible intervals
")
...
```{r contrast-ci, fig.width=12, fig.height = 6, warning = FALSE, message = FALSE}
