...
plotmat <- function(A, pos=NULL, curve=NULL, name=NULL, absent=0,
     ...
     segment.from = 0, segment.to = 1, latex = FALSE, arr.nudge.x = 0, arr.nudge.y = 0,
     ...)  {
  ...
  mm <- function(x) {
        ...
        return(matrix(nrow=ncomp, ncol=ncomp, x))
  }
  ...
  arr.nudge.x <- mm(arr.nudge.x)
  arr.nudge.y <- mm(arr.nudge.y)
  ...
  if (length(nonzero)>0)  {
    for (i in 1:nrow(nonzero))  {
      ...
      nudge.x    <- arr.nudge.x[ii[1], ii[2]]
      nudge.y    <- arr.nudge.y[ii[1], ii[2]]
      ...
      if (is.nan(angle))   {      #  pos1=pos2: self arrow
       ...
       if (ry==0)   {    # straight line
         ...
         if(cex.txt>0 && txt!= "")
           text(ell[1,1] + nudge.x, ell[1,2] + nudge.y, Parse(txt), adj=adj, cex=cex.txt, col=arr.tcol)
         TT <- rbind(TT, c(ell[1, 1] + nudge.x, ell[1, 2] + nudge.y, adj))
       }
    }   # end i
    GG <- c (GG, txt)
    RR <- c (RR, ry)}
