...
The goal is to understand the role of paternal care of offspring (male parental care) and sperm competition in driving the evolution of accessory glands. While male parental care is a fairly unambiguous binary trait, sperm competition is harder to classify.  Here we use spawning context (pair spawn1ing vs. group spawning) as a proxy for sperm competition. (**Note** from John Fitzpatrick: we should be careful how we phrase this, may want to replace "sperm competition" with "spawning behaviour" throughout. Leaving it alone for now, though.)
...
atab2 <- (merge(dtab, atab, by = 0)
    |> dplyr::select(desc, dAIC, dAICc, dBIC, dLogLik, df)
    |> arrange(dAIC)
)
...
gg_sum_gain <- ggplot(
    filter(ag_contr_gainloss, rate == "gain"),
    aes(x = exp(value), y = contrast)) +
    geom_violin(color="deeppink3", fill="deeppink3", alpha=0.7) +
    stat_summary(fun.data = "median_hilow",
                 geom = "errorbar",
                 ## width by trial and error; not sure what determines this?
                 position = position_dodge(width=0.875),
...
```{r multistochmap, echo=FALSE, cache=TRUE, eval = FALSE}
## slow, move this to targets?
tar_load(ag_model_pcsc)
set.seed(1)
...
## the very slow part
## (progress = TRUE only works for my hacked version of phytools (see github),
##  although I
obj1 <- densityMap(agSims, plot=FALSE, check = FALSE, progress = TRUE)
n <- length(obj1$cols)
...
if (FALSE) {
  ...
  cols1 <- list(care=c("#E49BFF","#8F09C0"), spawning=c("#A0D8EE","#3182bd"))
