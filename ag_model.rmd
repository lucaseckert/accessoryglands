...
tar_load(starts_with("ag_model"))
knitr::kable(as.data.frame(bbmle::AICtab(ag_model_indep, ag_model_pc, ag_model_sc, ag_model_pcsc,
                                         logLik = TRUE)),
             digits=1)
```
...
ggplot(
  all_ci,
  aes(term, exp(estimate), ymin = exp(lwr), ymax = exp(upr), colour = method, shape = method)
) +
...
sm0[[1]]$tip.label[] <- ""
phytools::plotSimmap(sm0[[1]], type="fan")
```
