...
Want {intercept, care, sperm comp, interaction} $\times$ {loss, gain, net gain}
```{r}
ag_mcmc1 <- as.mcmc(ag_mcmc0)
eff_names <- c(outer(c("intercept", "pc", "sc", "pcxsc"),
                     c("loss", "gain", "netgain"),
                     paste, sep = "_"))
contr <- matrix(0, ncol(ag_mcmc1), length(eff_names),
                dimnames = list(colnames(ag_mcmc1), eff_names))
write.csv(contr, "contr.csv")
```
