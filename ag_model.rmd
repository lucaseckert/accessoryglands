...
n<-length(obj1$cols)
obj1$cols[1:n]<-colorRampPalette(c("grey25","deeppink3"), space="Lab")(n)

smSum<-summary(sm1)
nodeProbs<-as.data.frame(smSum$ace)
nodeProbs$ag1<-rowSums(nodeProbs[,5:8])
nodeProbs$ag<-as.factor(round(nodeProbs$ag1, digits = 0))
agNodes<-rownames(subset(nodeProbs, ag==1))
agNodes

plot.new()
plot(ag_model_pcsc$phy)
nodelabels(pch = 19,agNodes)

plot(obj1,type="fan",ftype="off", lwd=2)
nodelabels(node = agNodes, pch = 19)
tiplabels(tip = agNodes, pch = 19)
