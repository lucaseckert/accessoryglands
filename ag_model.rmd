...
**fixme**: why is `Tylosurus_acus_melanotgus` but not in data?
...
gg_sum <- ggplot(ag_contr_gainloss, aes(x = exp(value), y = rate)) +
  ...
  geom_violin(aes(fill = phylo), alpha=0.6) +
    stat_summary(fun.data = "median_hilow",
                 geom = "linerange", aes(group=phylo),
...
Hmm, this says the results differ less from the priors than I would have thought / priors are more constraining than I thought (is that partly because the constraints represent sums of independent prior values, so they get 'shrunk'?)
