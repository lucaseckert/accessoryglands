...
```{r}
tot_evo <- sum(ag_compdata$phy$edge.length)
## log-hazard for 0.01 to 1000 transitions/tree
log(c(0.01,1000)/tot_evo)
## built in lower/upper bounds for corHMM
log(c(1e-9, 1e-2))
```

**FIXME**: scale total edge length to 1 of tree to 1 before fitting? *or* shift parameters by `log(tot_evo)` to get expected number of events scale (exponentiate and plot parameters with `scale_x_log10()` ?
...
```r
cmat <- read_csv("contr.csv", col_types = cols()) %>% dplyr::select(-parname) %>% as.matrix()
