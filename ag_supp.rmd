...
```{r image, message=FALSE, fig.width = 12, fig.height = 5}
pc <- gray(0.9)
...
```{r prior_spec, fig.width = 10, fig.height = 5}
...
ag_models <- paste0("ag_model_",
                    c("pcsc_add","pcsc","pc","sc","indep"))
fake_nobs <- 30 ## ?? something weird about targets evaluation:
                ## claims fake_nobs not found below ??
atab <- (bbmle::AICtab(ag_model_indep,
                      ag_model_pc,
                      ag_model_sc,
                      ag_model_pcsc,
                      ag_model_pcsc_add,
                      logLik = TRUE)
    ## keep rownames/don't convert to tibble
    |> transform(dAICc = (dAIC + 2*(df^2 + df)/(30-df-1)))
    |> transform(dAICc = dAICc - min(dAICc))
    |> transform(dBIC = dAIC - 2*df + log(30)*df)
    |> transform(dBIC = dBIC - min(dBIC))
)
dtab <- data.frame(desc = c("AG dep on PC, SC additively",
                            "AG dep on PC, SC & interaction",
                            "AG dep only on PC",
                            "AG dep only on SC",
                            "AG evol indep of PC, SC"),
                   row.names = ag_models)
atab2 <- (merge(dtab, atab, by = 0)
    |> dplyr::select(desc, dAICc, dAIC, dBIC, dLogLik, df)
    |> arrange(dAICc)
)
...
Missing confidence interval lines indicate that confidence intervals could not be computed; points at the edge of the graph (`sc_gain` and `pcxsc_loss`) indicate that the point estimates fell outside of the plotted range.

Intercepts are excluded (as being less biologically interesting, and using a different scale from the contrasts: hazards rather than hazard ratios).
