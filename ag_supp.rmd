...
```{r mcmc_pairs_flag, cache = TRUE}
include_mcmc_pairs <- FALSE
```
...
## Accessory gland search terms

[species name] OR [family name] OR accessory OR gland OR seminal OR vesicle OR testicular OR duct OR testes OR reproductive OR reproduction OR morphology OR anatomy OR histology

## Parental care and mating behaviour search terms

[species name] OR [family name] OR parental OR care OR reproduction OR reproductive OR spawn OR brood OR paternal OR mating OR breed OR reproduce

## ART search terms

[species name] OR [family name] OR sperm OR competition OR alternative OR reproductive OR tactic OR sneaker OR satellite OR guarder OR parental OR territorial OR bourgeois OR fertilize OR female OR mimic OR type OR male OR group OR spawning OR phase

# Descriptive statistics

**Note**: throughout the trait describing spawning mode (pair spawning vs. {group spawning or ART}) is described as "sc" (because we think of this trait has a proxy for sperm competition); "sm" is used in the main paper.
...
barplot_trans <- (. 
  %>% mutate(across(c(pc, sc), \(x) factor(x, levels = c(0:1, "?"))))
  %>% mutate(across(ag, \(x) factor(x, levels = 1:0))) ## reverse order to get ag=1 on top of bars
)
...
fix_all_ci <- function(x, levels = NULL) {
    ...
    if (!is.null(levels)) {
        ...
        x <- (x
            %>%  mutate(across(term, \(x) factor(x, levels = levels)))
        )
    }
    return(x)
}
...
(ggplot(prior_contr_ci)
    + aes(x = exp(estimate),
          ...
          xmax = exp(upr), y = term)
    + geom_pointrange()
    + labs(y = "", x = "transition rate or transition rate ratio")
    + scale_x_log10()
...
```{r get_data_sizes, echo = FALSE}
tar_load(ag_compdata_tb_nomiss)
tar_load(ag_compdata_tb)
n_full <- nrow(ag_compdata_tb$data)
n_nomiss <- nrow(ag_compdata_tb_nomiss$data)
```
...
- `mcmc_tb_nomiss`: as `mcmc_tb`, but using only species with complete data for all three traits (this includes less than half of the full data set, `r n_nomiss` vs. `r n_full` species)
...
## Now do the same thing, but substitute in the CONTRASTS
...
all_contr_ci <- (fix_all_ci(all_contr_ci, levels = contr_levs)
    ## don't want prior predictive sims, or net-change estimates, or intercept terms
    |> filter(!grepl("net", term),
              !grepl("intercept", term),
              method != "priorsamp",
              !is.na(term))
    |> mutate(across(method, \(x) factor(x, levels =rev(method_levs))))
)
...
gg_ci1C  <- (gg_ci0 %+% all_contr_ci
    ...
    + aes(shape = method)
    + scale_shape_manual(values = c(scales::shape_pal(solid=TRUE)(6),9,10,11))
)

print(gg_ci1C)
```

```{r sens_plot_miss, echo=FALSE, message = FALSE}
## save plot for specific primary/nomiss comparison
gg_nmiss <- gg_ci1C %+% filter(all_contr_ci, method %in% c("mcmc_tb", "mcmc_tb_nomiss"))
ggsave(gg_nmiss, file = "sensitivity_nmiss.pdf")
```
...
- `mcmc_tb` vs. `mcmc_tb_nomiss`: interestingly, selecting only species with complete data has a moderately large effect on our results. In particular, the estimated sign of the effect of spawning mode on gain (`sc_gain`) flips from negative to positive (and indeed *clearly* positive), while the effect of paternal care on gain, while still positive, decreases and becomes unclear. Furthermore, estimates based on the no-missing-data subset suggest that group spawning also decreases the loss rate of accessory glands.
...
```{r bayes_pairs2, fig.width=10, fig.height=10, cache=TRUE, dependson = "mcmc_pairs_flag"}
if (include_mcmc_pairs) {
...
```{r bayes_pairs_tb, fig.width=10, fig.height=10, cache=TRUE, dependson = "mcmc_pairs_flag"}
if (include_mcmc_pairs) {
...
```{r bayes_pairs_full, fig.width=10, fig.height=10, cache=TRUE, dependson = "mcmc_pairs_flag"}
if (include_mcmc_pairs) {
...
```{r bayes_pairs_tb_nogainloss, fig.width=10, fig.height=10, cache=TRUE, dependson = "mcmc_pairs_flag"}
if (include_mcmc_pairs) {
