...
```{r image, message=FALSE, fig.width = 12, fig.height = 5, fig.cap = "Transition matrix for full and reduced (independent evolution of spawning mode and male care) models. Note that estimated rates (represented by grid square colours) for 24-parameter model are extremely uncertain; see sensitivity analysis (last section)."}
pc <- gray(0.9)
...
```{r contrast-diag, echo = FALSE, out.width = 500, out.height = 500}
knitr::include_graphics("flowfig2.png")
```
...
```
```{r lrtest-plot, fig.width = 7, fig.height = 10}
plotmat(A, curve = 0,
...
We can look at the parameter estimates from MLE fits, although
