...
Reduced (12-parameter) and unreduced (24-parameter) models:

```{r image, message=FALSE, fig.width = 12, fig.height = 5}
cowplot::plot_grid(plot(ag_model_pcsc),
                   plot(ag_model_full), nrow=1)
```
...
```{r contrast_ci, fig.width=12, fig.height = 6, warning = FALSE, message = FALSE}
...
gg_ci1_nowaldC <- (gg_ci1C
    %+% dplyr::filter(all_contr_ci, method != "model_pcsc")
)
cowplot::plot_grid(gg_ci1C, gg_ci1_nowaldC, nrow = 1)
```

**Conclusions**

- `priorsamp`: the prior distributions on loss and gain are neutral (mean = 1) as promised, with 95% ranges of approximately (1/10, 10) [seems reasonable]. Priors on intercepts are different from 1 (as expected).
- `model_pcsc` (blue-green): these estimates are all over the place, we can't really practically estimate the parameters/CIs
- `model_pcsc_prior`: for the most part these likelihood-based fits look similar to the Bayesian solutions (i.e., most of the frequentist vs Bayes distinction lies in the *regularization* step, imposing reasonable priors, rather than in the more sophisticated handling of uncertainty)
- `mcmc_0` (basic model with single/full phylogeny): slightly harder to see signals (confidence limits overlap 1 more), because we are leaving out information for a chunk of the phylogeny
- `mcmc_tb`: can see a strong signal of increased AG gain with PC, weak signal (p = 0.06 from above) of *decreased* AG gain with SC
- `mcmc_tb_nogainloss` (yellow-green): leaving out the gain/loss priors doesn't make a huge difference; it slightly weakens the SC signal and strengthens the PC signal
- full pc/sc likelihood-based model alone gives us huge CIs, no confidence

## fixme

- figure out what's up with missing CIs for `model_pcsc`, `model_pcsc_prior`
- profile CIs for ML fits? (probably v. slow/wonky ...)
- include full 24-parameter fits (need to work out contrasts!)

### cosmetic

- reorder levels
- partition as in main plots (facet by sc/pc/interaction/intercept?)


```{r check_contrasts, eval = FALSE}
## Check contrast tidier calculations:
## (Values of estimates not quite identical: mean vs median snuck in somewhere or ??? Not too worried though.)
tar_load(contr_long_ag_mcmc_tb)
c1 <- contr_long_ag_mcmc_tb %>% group_by(contrast, rate) %>% summarise(m = median(value), m_lwr = quantile(value, 0.025), m_upr = quantile(value, 0.975), .groups = "drop")
c2 <- all_contr_ci %>% filter(method == "mcmc_tb") %>% dplyr::select(term, estimate, lwr, upr) %>% separate(term, sep = "_", into = c("contrast", "rate"))
full_join(c1, c2)
```
