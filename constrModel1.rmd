...
effect of care on the rate of accessory gland evolution
```{r}
m1L <- emdbook::lump.mcmc.list(m1)
careToAG <- ((m1L[,11]-m1L[,7]+m1L[,12]-m1L[,8])/2)-((m1L[,9]-m1L[,3]+m1L[,10]-m1L[,5])/2)
careCI<-quantile(careToAG, c(0.025,0.975))
hist(careToAG, breaks=100)
```

effect of sperm comp on the rate of accessory gland evolution
```{r}
spermCompToAG<-((m1L[,10]-m1L[,5]+m1L[,12]-m1L[,8])/2)-((m1L[,11]-m1L[,7]+m1L[,9]-m1L[,3])/2)
spermCompCI<-quantile(spermCompToAG, c(0.025,0.975))
hist(spermCompToAG, breaks=100)
```
interactions?
```{r}
int1<-((m1L[,12]-m1L[,8])-(m1L[,10]-m1L[,5]))-((m1L[,11]-m1L[,7])-(m1L[,9]-m1L[,3]))
int1CI<-quantile(int1,c(0.025,0.975))
hist(int1, breaks = 100)

int2<-((m1L[,12]-m1L[,8])-(m1L[,9]-m1L[,3]))-((m1L[,10]-m1L[,5])-(m1L[,11]-m1L[,7]))
int2CI<-quantile(int2, c(0.025,0.975))
